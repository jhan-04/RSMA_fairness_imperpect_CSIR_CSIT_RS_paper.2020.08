function  [pc,pk]=Alg_1_noRS(Nt,K,pc,pk,H_m,Pt,M,N0)
%%%
n=2;
A=[];
A(2)=-10;
...
while abs((A(n)-A(n-1))/A(n))>=e
    ...
    for k=1:K
        for i=1:K
            a(k,i)=P((Nt*(i-1)+1):Nt*i,1)'*psi_k(:,:,k)*P((Nt*(i-1)+1):Nt*i,1);
        end
        c(k)=real(f_k(:,k)'*P((Nt*(k-1)+1):Nt*k,1));
    end
    
    
    %xx=xi_c+sum(sum(a))+sum(N0*t_k+u_k-v_k)+sum(-2.*c);
    xx=sum(sum(a))+sum(N0*t_k+u_k-v_k)+sum(-2.*c);
    minimize(xx)
    subject to
    %     for k=1:K
    %         for i=1:k
    ...
    A(n)=xx;
    if A(n)>A(n-1)&&n>3
        fprintf('NOT monotonically decreasing %d in MMSE-noRS \n',A(n)-A(n-1))
        A
    end
