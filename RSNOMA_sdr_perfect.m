...
function  [MA_x,t_x, P1_x,P2_x, Pc_x,Rs_x,fc_x,t2,Rs_2,fc_2]=RSNOMA_sdr_perfect(P,h1,h2,rho)
...
if rho==0
    ...
    t_x=0;
    h1_=h1/sqrt(1+norm(h1)^2*rho*P*t_x);
    
    h2_=h2;
    Rc1=log2(1+abs(h1_'*f_c)^2*(1- t_x)*P);
    Rc2=log2(1+abs(h2_'*f_c)^2*(1- t_x)*P);
    Rs_x=min(Rc1,Rc2);
    ...
    t2_2=round((K-1)/(norm(h1)^2*rho*P),4);  
    %%%%%
