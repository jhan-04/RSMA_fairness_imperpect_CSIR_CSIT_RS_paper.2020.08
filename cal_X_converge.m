function  [X,result_set,converg]=cal_X_converge(Nt,M,Pt,A,B,C,D)
...
while ~stop
    ...
    s
    % until
    sss(s)=norm(b0(:,s)-b0(:,s-1))^2+norm(d0(:,s)-d0(:,s-1))^2;
    if s>10%norm(b0(:,s)-b0(:,s-1))^2+norm(d0(:,s)-d0(:,s-1))^2<ee%norm(result(s)-result(s-1))^2<0.0001
        stop=1;
    end
