...
BETA=[0.001:0.05:0.2]%[0.01,0.05, 0.1, 0.2,0.3]%[0.05:0.05:0.3];
...
for i=1:sample
    ...
    j=0;
    for beta=BETA%[0.0001,0.001,0.01,0.05,0.1,0.2]
        j=j+1;
% % % % %         h1(:,i)=sqrt(1-beta).*h1(:,i);
% % % % %         h2(:,i)=sqrt(1-beta).*h2(:,i);
        
        
        %%%%%%%%%%NRS_NOMA
        [MA_Ri(i,j),t_Ri(i,j), P1_Ri(i,j),P2_Ri(i,j), Pc_Ri(i,j),Rs_Ri(i,j),fc_Ri(:,i,j),t_Ni(i,j),Rs_Ni(i,j),fc_Ni(:,i,j)]=RSNOMA_kkt_imperfect(P,h1(:,i),h2(:,i),rho,beta);
        [MA_Rp(i,j),t_Rp(i,j), P1_Rp(i,j),P2_Rp(i,j), Pc_Rp(i,j),Rs_Rp(i,j),fc_Rp(:,i,j),t_Np(i,j),Rs_Np(i,j),fc_Np(:,i,j)]=RSNOMA_kkt_perfect(P,h1(:,i),h2(:,i),rho);
        %%%%%%MRT
        [MA_Nim(i,j),t_Nim(i,j), P1_Nim(i,j),P2_Nim(i,j), Pc_Nim(i,j),Rs_Nim(i,j),fc_Nim(:,i,j)]=NOMA_MRT_imperfect(P,h1(:,i),h2(:,i),rho,beta);
        
        
        %%%%%%%%%%%%%%%%%%%%%%%%%SDMA
        [MA_Si(i,j),t_Si(i,j), P1_Si(i,j),P2_Si(i,j), Pc_Si(i,j),Rs_Si(i,j),fc_Si(:,i,j)]=SDMA_imperfect(P,h1(:,i),h2(:,i),rho,beta);
        [MA_Sp(i,j),t_Sp(i,j), P1_Sp(i,j),P2_Sp(i,j), Pc_Sp(i,j),Rs_Sp(i,j),fc_Sp(:,i,j)]=SDMA_perfect(P,h1(:,i),h2(:,i),rho);

        %%%%%%%OMA
        [MA_Oi(i,j),t_Oi(i,j), P1_Oi(i,j),P2_Oi(i,j), Pc_Oi(i,j),Rs_Oi(i,j),fc_Oi(:,i,j)]=OMA_imperfect(P,h1(:,i),h2(:,i),rho,beta);
        [MA_Op(i,j),t_Op(i,j), P1_Op(i,j),P2_Op(i,j), Pc_Op(i,j),Rs_Op(i,j),fc_Op(:,i,j)]=OMA_perfect(P,h1(:,i),h2(:,i),rho);
       %%%%%%%MRT
        [MA_Oim(i,j),t_Oim(i,j), P1_Oim(i,j),P2_Oim(i,j), Pc_Oim(i,j),Rs_Oim(i,j),fc_Oim(:,i,j)]=OMA_MRT_imperfect(P,h1(:,i),h2(:,i),rho,beta);
       
        %%%%%%multicasting
        [MA_mi(i,j),t_mi(i,j), P1_mi(i,j),P2_mi(i,j), Pc_mi(i,j),Rs_mi(i,j),fc_mi(:,i,j)]=multi_kkt_imperfect(P,h1(:,i),h2(:,i),rho,beta);
        [MA_mp(i,j),t_mp(i,j), P1_mp(i,j),P2_mp(i,j), Pc_mp(i,j),Rs_mp(i,j),fc_mp(:,i,j)]=multi_kkt_perfect(P,h1(:,i),h2(:,i),rho);
        
        
        
        %%%%%%%%%%%%%%%%%%caculate GMI
        %%%%%%%%%%RS_NOMA
        Rs__Ri(i,j)=cal_GMI(P,t_Ri(i,j), P1_Ri(i,j),P2_Ri(i,j), Pc_Ri(i,j),fc_Ri(:,i,j),h1(:,i),h2(:,i),rho,beta);
        Rs__Rp(i,j)=cal_GMI(P,t_Rp(i,j), P1_Rp(i,j),P2_Rp(i,j), Pc_Rp(i,j),fc_Rp(:,i,j),h1(:,i),h2(:,i),rho,beta);
        Rs__Ni(i,j)=cal_GMI(P,t_Ni(i,j), t_Ni(i,j)*P,0, (1-t_Ni(i,j))*P,fc_Ni(:,i,j),h1(:,i),h2(:,i),rho,beta);
        Rs__Np(i,j)=cal_GMI(P,t_Np(i,j), t_Np(i,j)*P,0, (1-t_Np(i,j))*P,fc_Np(:,i,j),h1(:,i),h2(:,i),rho,beta);
        
        %%%%%%%%%%%%%%%%%%%%%%%%%SDMA
        Rs__Si(i,j)=cal_GMI(P,t_Si(i,j), P1_Si(i,j),P2_Si(i,j), Pc_Si(i,j),fc_Si(:,i,j),h1(:,i),h2(:,i),rho,beta);
        Rs__Sp(i,j)=cal_GMI(P,t_Sp(i,j), P1_Sp(i,j),P2_Sp(i,j), Pc_Sp(i,j),fc_Sp(:,i,j),h1(:,i),h2(:,i),rho,beta);
        
        %%%%%%%%%%%%%%%%%%%OMA
        Rs__Oi(i,j)=cal_GMI(P,t_Oi(i,j), P1_Oi(i,j),P2_Oi(i,j), Pc_Oi(i,j),fc_Oi(:,i,j),h1(:,i),h2(:,i),rho,beta);
        Rs__Op(i,j)=cal_GMI(P,t_Op(i,j), P1_Op(i,j),P2_Op(i,j), Pc_Op(i,j),fc_Op(:,i,j),h1(:,i),h2(:,i),rho,beta);

        %%%%%%%%%%%%%%%multi
        Rs__mi(i,j)=cal_GMI(P,t_mi(i,j), P1_mi(i,j),P2_mi(i,j), Pc_mi(i,j),fc_mi(:,i,j),h1(:,i),h2(:,i),rho,beta);
        Rs__mp(i,j)=cal_GMI(P,t_mp(i,j), P1_mp(i,j),P2_mp(i,j), Pc_mp(i,j),fc_mp(:,i,j),h1(:,i),h2(:,i),rho,beta);
        
        
        

        
                fprintf('=======sample=%d===================beta=%d===================\n\n',i,beta)
...
bar(BETA,CASE,'stacked')
legend('SDMA','NOMA','OMA','multicasting','RS')
xlabel('beta(channel error covariance parameter)')
title(['SNR of user1=',num2str(SNR),', SNR of user 2=',num2str(SNR+gam_dB)])

%%%%average of GMI with sample channel
figure(2)
hold on
%%RS
plot(BETA,mean(Rs__Ri),'r')
plot(BETA,mean(Rs__Rp),'r--o')
%%NOMA
plot(BETA,mean(Rs__Ni),'b')
plot(BETA,mean(Rs__Np),'b--o')
plot(BETA,mean(Rs_Nim),'b--d')
%SDMA
plot(BETA,mean(Rs__Si),'g')
plot(BETA,mean(Rs__Sp),'g--o')
%OMA
plot(BETA,mean(Rs__Oi),'c')
plot(BETA,mean(Rs__Op),'c--o')
%MRT
plot(BETA,mean(Rs_Oim),'c--+')
%multicasting
plot(BETA,mean(Rs__mi),'m')
plot(BETA,mean(Rs__mp),'m--o')
...
xlabel('beta(channel error covariance parameter)')
title(['SNR of user1=',num2str(SNR),', SNR of user 2=',num2str(SNR+gam_dB),', BETA=',num2str(BETA(k))])
