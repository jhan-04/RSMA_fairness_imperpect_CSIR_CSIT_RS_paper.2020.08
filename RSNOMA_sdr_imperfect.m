...
function  [MA_x,t_x, P1_x,P2_x, Pc_x,Rs_x,fc_x,t2,Rs_2,fc_2]=RSNOMA_sdr_imperfect(P,h1,h2,rho,beta)
...
if (fc_1'*A*fc_1)>=(fc_1'*B*fc_1)%Rc1>Rc2
    ...
    Gam=((1+beta*P)/rho)*(1/norm(hb)^2-1/norm(ha)^2);
    b=norm(ha)^2*rho*P/2;
    a=(1+beta*P)+(Gam/P)*b;
    d=norm(hb)^2*rho*P/2-abs(hb'*fc_1)^2*P;
    c=(1+beta*P)-(Gam/P)*d+abs(hb'*fc_1)^2*(P-Gam);
