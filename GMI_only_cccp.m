function  [p_opt,result_cccp,GMI_cccp]=GMI_only_cccp(Nt,K,H_h,Pt,N0,sigma_e,num_interation)
...
%         [real(p0(:,s-1)'*C(:,:,i)), -imag(p0(:,s-1)'*C(:,:,i))]*[real(p);imag(p)]>=exp(c(i));
        ...
        [real(2.*p0(:,s-1)'*A(:,:,i)), imag(2.*p0(:,s-1)'*A(:,:,i))]*[imag(p);real(p)]-real(p0(:,s-1)'*A(:,:,i)*p0(:,s-1))>=0;
        [real(2.*p0(:,s-1)'*C(:,:,i)), imag(2.*p0(:,s-1)'*C(:,:,i))]*[imag(p);real(p)]-real(p0(:,s-1)'*C(:,:,i)*p0(:,s-1))>=0;
