...
for i=1:sample
    ...
    for j=1:1:length(SNR)
        ...
        %%%%%%NOMA_two user
        [P_noma,pc_noma,pk_noma,GMI_noma(i,j),SR_noma(i,j)]=GMI_NOMA_two_user(Nt,K,H_h,H_h,Pt,M,N0,sigma_e);
        ...
        %%%%%%%MRT
        [MA_Oim1(i,j),t_Oim1(i,j), P1_Oim1(i,j),P2_Oim1(i,j), Pc_Oim1(i,j),Rs_Oim1(i,j),fc_Oim1(:,i,j)]=OMA_MRT_one_imperfect(Pt,h1(:,i),h2(:,i),rho,sigma_e2);
...
plot(SNR,mean(SR),'-sr','LineWidth',2,...
    'MarkerSize',8)
plot(SNR,mean(SR_e),'--r>','LineWidth',1.5,...
    'MarkerSize',8)

plot(SNR,mean(SR_sdma),'b-.s','LineWidth',1.5,...
    'MarkerSize',8)
plot(SNR,mean(SR_sdma_e),'b--^','LineWidth',1.5,...
    'MarkerSize',8)
...
plot(SNR,mean(SR_mrt),'y-d','LineWidth',1.5,...
    'MarkerSize',8)
plot(SNR,mean(SR_mrt_e),'y--<','LineWidth',1.5,...
    'MarkerSize',8)

plot(SNR,mean(SR_noma),'m--d','LineWidth',1.5,...
    'MarkerSize',8)
plot(SNR,mean(SR_noma_e),'m--<','LineWidth',1.5,...
    'MarkerSize',8)
...
legend('RS',' RS_{no error info}','SDMA','SDMA_{no error info}','RS-ZF','RS-ZF_{no error info}',...
    'RS-MRT','RS-MRT_{no error info}','NOMA','NOMA_{no error info}','RS-ZF','RS-ZF_{no error info}',...
    'NOMA-ZF','NOMA-ZF_{no error info}','SDMA-ZF','SDMA-ZF_{no error info}','OMA-MRT1','OMA-MRT2','multicasting','multicasting_{no error info}')
